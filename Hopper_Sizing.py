...
# --- 1. DATA LOADING (Standard Hopper Table & Froth Factors) ---
@st.cache_data
def load_standard_hoppers():
    # Transcribed representative rows from the provided image for standard sizes
    data = {
...
def plot_hopper(hopper, shape, ll_lvl, hh_lvl):
    ...
    # Extract dimensions (convert mm to m for plotting)
...
# --- 3. STREAMLIT UI ---
st.set_page_config(layout="wide", page_title="Pump Hopper MTO Calculator")
...
with st.sidebar:
    ...
    min_res = st.number_input("Min Residence Time (sec)", value=45.0) / 60  # convert to min internally
...
# Shape Adjustment: Standard table is based on Round. 
# A Square hopper of Width = Diameter holds ~27% more (4/pi).
# To find the right standard dimensions for a square hopper, we divide required volume by (4/pi).
shape_factor = 1.0 if shape == "Round" else (math.pi / 4)
lookup_vol = req_live_vol * shape_factor

# 2. Select Standard Hopper
try:
...
# Actual Live Volume Provided (adjusting back if square)
...
with col2:
    ...
    # Calculate Levels (converted to meters for display)
    cone_h_m = selected_hopper['Cone Height (mm)'] / 1000
    cyl_to_notch_m = selected_hopper['Height Cyl to Notch (mm)'] / 1000
    
    # HH Level is at the Notch
    hh_lvl = cone_h_m + cyl_to_notch_m
    
    # LL Level (Assume ANSI HI submergence minimum, typically top of cone for standard sizing, or specific offset)
    # Using top of cone as LL base approximation for this script 
    ll_lvl = cone_h_m 
    ...
    st.write(f"- **Overflow Box Depth:** {selected_hopper['OF Box Depth Below Notch (mm)']} mm")
    st.write(f"- **Overflow Box Projection:** {selected_hopper['OF Box Projection (mm)']} mm")
